...
from orionmdcore.standards import MDStageTypes, MDEngines, MDStageNames, Fields

from orionplatform.ports import RecordInputPort, RecordOutputPort

from orionmdcore.mdrecord import MDDataRecord

from orionmdcore.cubes.mdengines.utils import (
    md_simulation,
...
from orionmdcore.cubes.mdengines.utils import schedule_cycles
...
class MDProxyCube(RecordPortsMixin, ComputeCube):
    ...
    cube_max_run_time = parameters.DecimalParameter(
        "cube_max_run_time", default=0.05, help_text="Max Cube Running Time in hrs"
    )
        ...
        try:
            ...
            if not record.has_field(Fields.cycle_id):
                ...
                for iteration, (time, frame) in schedule.items():
                    info_str += "{:4d}  {:8.4f}  {:8d}  {}\n".format(
                        iteration, time, frame, "*" if iteration == 0 else ""
                    )
                    tot_time += time
                    tot_frames += frame
                info_str += 27 * "-" + "\n"
                ...
                if str(next_cycle_id) not in schedule:
                    ...
                    for iteration, (time, frame) in schedule.items():
                        info_str += "{:4d}  {:8.4f}  {:8d}  {}\n".format(
                            ...
                            time,
                            frame,
                            "*" if int(iteration) == next_cycle_id else "",
                        )
                        tot_time += time
                        tot_frames += frame
                    info_str += 27 * "-" + "\n"
